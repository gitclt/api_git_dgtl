...
INNER JOIN 
    tbl_projectmember m ON m.emp_id = e.id
LEFT JOIN 
    tbl_task_emp te ON te.emp_id = e.id 
LEFT JOIN 
    tbl_task t ON te.task_id = t.id
LEFT JOIN 
    tbl_log_time lt ON lt.task_id = t.id AND lt.created_user_id = te.emp_id  
WHERE 
    m.project_id = '"+project_id+"' GROUP BY  e.name, e.designation_id, e.email, d.name";
