...
public partial class api_catlog_add_wishlist : System.Web.UI.Page
{
    ...
    public void insert()
    {
        ...
        foreach (var data in dataResponseList)
        {
          ...
          VALUES ('" + data.tasklist_id + "', '" + data.taskname + "', " +
            (string.IsNullOrEmpty(data.startdate) ? "NULL" : "'" + data.startdate + "'") + ", " +
            (string.IsNullOrEmpty(data.duedate) ? "NULL" : "'" + data.duedate + "'") + ", '" +
            data.estimated_time + "', '" + data.status + "', '" + data.pro_id + "', '" + data.priority_id + "');";
            ...
            else if (data.type == "delete")
            {
                ...
                query1 += @"delete from tbl_task_emp
                       WHERE task_id = '" + data.id + "'";
            ...
            {
               string query3 = @"update  tbl_task set tasklist_id='"+data.tasklist_id+ "',taskname='"+data.taskname + "',startdate='"+data.startdate + "'," +
                    "duedate='"+data.duedate + "',estimated_time='"+data.estimated_time + "',status='"+data.status + "',pro_id='"+data.pro_id + "',priority_id='"+data.priority_id + "' WHERE id = '" + data.id + "'";

            
                int status = cc.Insert(query3);
