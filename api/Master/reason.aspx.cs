...
public partial class api_country_add : System.Web.UI.Page
{
    ...
    public void insert()
    {
        ...
        foreach (var data in DataResponse)
        {
            ...
            if (data.type == "add")
            {
                 querry1 = @"insert into tbl_reason(name,deleted_status) values('" + data.name + "',0)";
                message = "Data added successfully.";

            }
            else if(data.type=="edit")
            {
                querry1 = @"update tbl_reason set name='" + data.name+ "' where id=" + data.id+" ";
                message = "Data updated successfully.";

            }
            else if(data.type == "delete")
            {
                querry1 = @"update  tbl_reason  set deleted_status=1,deleted_by='" + data.deleted_by+"',deleted_on=getdate() where id=" + data.id + " ";
