...
public partial class api_country_add : System.Web.UI.Page
{
    ...
    WITH Results AS (
        SELECT 
            o.id AS oid,
            o.pm_id,o.pm_remark,o.pm_delivery,
            o.order_no,
            o.addedon,
            o.addedby,
            o.customer,
...
c.first_name as customername,
    ...
    DataSet ds = cc.joinselect(query);
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                json += "{'order_number':'" + ds.Tables[0].Rows[i]["order_no"].ToString() + "','id':'" + ds.Tables[0].Rows[i]["oid"].ToString() + "','addedby':'" + ds.Tables[0].Rows[i]["addedby"].ToString() + "'," +
                    "'employee':'" + ds.Tables[0].Rows[i]["employee"].ToString() + "','date':'" + ds.Tables[0].Rows[i]["addedon"].ToString() + "','customer_id':'" + ds.Tables[0].Rows[i]["customer"].ToString() + "'," +
                    "'email':'" + ds.Tables[0].Rows[i]["email"].ToString() + "','mobile':'" + ds.Tables[0].Rows[i]["mobile"].ToString() + "','total_items':'" + ds.Tables[0].Rows[i]["total_items"].ToString() + "','customername':'" + ds.Tables[0].Rows[i]["customername"].ToString() + "'," +
                    "'total_amount':'" + ds.Tables[0].Rows[i]["total_amount"].ToString() + "','packing_assign_status':'" + ds.Tables[0].Rows[i]["packing_assign_status"].ToString() + "','packing_status':'" + ds.Tables[0].Rows[i]["packing_status"].ToString() + "'," +
                    "'production_assign_status':'" + ds.Tables[0].Rows[i]["production_assign_status"].ToString() + "','production_status':'" + ds.Tables[0].Rows[i]["production_status"].ToString() + "','remark':'" + ds.Tables[0].Rows[i]["remark"].ToString() + "'," +
                    "'pm_delivery':'" + ds.Tables[0].Rows[i]["pm_delivery"].ToString() + "','pm_remark':'" + ds.Tables[0].Rows[i]["pm_remark"].ToString() + "','pm_id':'" + ds.Tables[0].Rows[i]["pm_id"].ToString() + "'},";
            }
            json = json.TrimEnd(',');
            json += "]}";
