...
public partial class api_country_add : System.Web.UI.Page
{
    ...
    public void insert()
    {
        ...
        foreach (var data in DataResponse)
        {
            ...
            if (data.type == "add")
            {
                 querry1 = @"insert into tbl_gsm (name,delete_status) values('" + data.name + "',0)";
                message = "Data added successfully.";

            }
            else if(data.type=="edit")
            {
                querry1 = @"update tbl_gsm set name='" + data.name+"' where id="+data.id+" ";
                message = "Data updated successfully.";

            }
            else if(data.type == "delete")
            {
                querry1 = @"delete from tbl_gsm  where id=" + data.id + " ";
